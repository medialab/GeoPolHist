...
with open('GeoPolHist_entities_status_over_time.csv', 'r', encoding='utf8') as r:
    ...
    def get_priority_status(status): return sorted(
        (o['status'] for o in status), key=lambda o: -1*translate_GPH_status[o]['priority_order'])[0]
    for code, entity in COW_entities.items():
        ...
        for s_p, e_p in zip(periods, periods[1:]):
            ...
            if start_status and end_status:
                status_stock_period_agg["%s %s-%s" % (start_status, s_p['start_year'], s_p['end_year'])]["%s %s-%s" % (
                    end_status, e_p['start_year'], e_p['end_year'])] += 1
            # if one status is not know the entity disapeared => no link
        for p, n in zip(year_status, year_status[1:]):
            previous_status = translate_GPH_status[get_priority_status(
                p[1])]['group']
            next_status = translate_GPH_status[get_priority_status(
                n[1])]['group']
    ...
    with open(os.path.join(OUTPUT_PATH, 'status_transitions_by_periods.csv'), 'w', encoding='utf8') as f:
        transitions_csv = csv.DictWriter(
            f, fieldnames=['period', 'transition', 'nb'])
    ...
    with open(os.path.join(OUTPUT_PATH, 'GeoPolHist_entities_over_time.csv'), 'w', encoding='utf8') as o:
        ...
        for number, e in COW_entities.items():
            ...
            for y in range(minYear, maxYear+1):
                if y in e['years']:
                    status_by_priority = sorted(
                        (o['status'] for o in e['years'][y]), key=lambda o: -1*translate_GPH_status[o]['priority_order'])
                    line[y] = " | ".join(
                        [translate_GPH_status[o]["slug"] for o in status_by_priority])
                    the_status = status_by_priority[0]
                    COW_types_by_year[the_status][y] += 1
                else:
                    line[y] = ''
            output.writerow(line)
    with open(os.path.join(OUTPUT_PATH, 'GeoPolHist_status_over_time.csv'), 'w', encoding='utf8') as o:
        ...
        for ctype, years in sorted(COW_types_by_year.items(), key=lambda o: translate_GPH_status[o[0]]['priority_order']):
            ...
            for y in range(minYear, maxYear+1):
                ...
                if translate_GPH_status[ctype]['group'] != "":
                    group_by_year[translate_GPH_status[ctype]
                                  ['group']][y] += line[y]
            type_by_year[ctype] = line
            output.writerow(line)

        with open(os.path.join(OUTPUT_PATH, 'GeoPolHist_statu_group_over_time.csv'), 'w', encoding='utf8') as f:
            output = csv.DictWriter(
                f, fieldnames=["GPH_status_group"]+list(range(minYear, maxYear+1)))
            output.writeheader()
